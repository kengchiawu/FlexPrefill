...
rouge_file_path = current_file_path.parent / "rouge.py"
ROUGE_SCORER = evaluate.load(str(rouge_file_path))
...
def get_score_one_kv_retrieval(items):
    ground_truth, prediction = items
    ground_truth = ground_truth.replace("===answerspliter===", "")
    for c in ["\n", ":", '"', "'", ".", ",", "?", "!", "{", "}"]:
...
def get_score_one_passkey(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    ground_truth = ground_truth_list[0]
    if ground_truth == first_int_match(prediction):
        score = 1
    else:
        score = 0
    return score
...
def get_score_one_number_string(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    ground_truth = ground_truth_list[0]
    if ground_truth == first_int_match(prediction):
...
def get_score_one_code_run(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    ground_truth = int(ground_truth_list[0])
...
def get_score_one_code_debug(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")

    prediction = prediction.strip()
    label_c = ground_truth_list[1]
    fn_name = ground_truth_list[0]
    if prediction[:2] in [f"{label_c}.", f"{label_c}:"]:
        return True
    ...
    for prefix in ans_prefixes:
        idx = prediction.find(prefix)
        if idx == -1:
            continue
        # The prediction ends with this prefix
        if len(prediction) < idx + len(prefix) + 1:
            return 0
        prediction = prediction[idx + len(prefix) + 1 :]
        for s in [label_c, fn_name]:
...
def get_score_one_math_find(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    ground_truth = int(float(ground_truth_list[0]))
    # Find first int or float
    first_num = re.search(r"\d+\.\d+|\d+", prediction)
    if first_num is None:
        return False
    first_num = first_num.group(0).strip()
    try:
        return int(float(first_num)) == ground_truth
    except:
        return 0
...
def get_score_one_longdialogue_qa_eng(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
...
def get_score_one_longbook_choice_eng(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    # Just use the first letter as the prediction
    prediction = prediction.strip()
    ...
    if prediction[0] in "ABCD":
        return prediction[0] in ground_truth_list
    ...
    for prefix in ans_prefixes:
        ...
        if len(prediction) < idx + len(prefix) + 1:
            return 0
        after_prefix = prediction[idx + len(prefix) + 1 :]
...
def get_score_one_longbook_qa_eng(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    return qa_f1_score(prediction, ground_truth_list)
...
def get_score_one_longbook_sum_eng(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    ground_truth = ground_truth_list[0]

    score = ROUGE_SCORER.compute(
        predictions=[prediction], references=[ground_truth], use_aggregator=False
    )
...
def get_score_one_longbook_qa_chn(items):
    ground_truth_list, prediction = items
    ground_truth_list = ground_truth_list.split("===answerspliter===")
    return qa_f1_score_zh(prediction, ground_truth_list)
